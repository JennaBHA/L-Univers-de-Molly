...
const AlimentationPage = () => {
  ...
  const ALIMENTATION_ID = 1;

  // Fetch Filters
  useEffect(() => {
    const fetchFilters = async () => {
      try {
        const [animalsData, subCatsData] = await Promise.all([
          productService.getAnimals(),
          productService.getSubCategories(ALIMENTATION_ID),
        ]);
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        const filters = {
          categoryId: selectedType !== "all" ? selectedType : ALIMENTATION_ID,
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div>
        <div className="max-w-7xl mx-auto px-6 py-6">
          <Link
            to="/"
            className="inline-flex items-center gap-2 text-gray-600 hover:text-orange-600 font-medium transition-colors mb-4"
          >
            <ArrowLeft size={18} />
            Retour
          </Link>
          ...
          <div className="bg-white rounded-lg p-5 shadow-sm border border-gray-200">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {subCategories.map((subCat) => (
                <button
                  key={subCat.id}
                  onClick={() => setSelectedType(subCat.id)}
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                    selectedType === subCat.id
                      ? "bg-orange-600 text-white"
                      : "bg-gray-100 text-gray-700 hover:bg-gray-200"
                  }`}
                >
          ...
          <div>
            ...
            {(selectedAnimal !== "all" || selectedType !== "all") && (
              <div className="flex gap-2 mt-2">
                {selectedAnimal !== "all" && (
                  <span className="inline-flex items-center gap-1.5 bg-orange-100 text-orange-700 px-3 py-1 rounded-md text-xs font-medium">
                ...
                {selectedType !== "all" && (
                  <span className="inline-flex items-center gap-1.5 bg-orange-100 text-orange-700 px-3 py-1 rounded-md text-xs font-medium">
                    {subCategories.find((f) => f.id === selectedType)?.name}
                    <button
                      onClick={() => setSelectedType("all")}
                      className="hover:text-orange-900"
                    >
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-6 py-3 rounded-lg font-semibold transition-all ${
                    currentPage === 1
                      ...
                      : "bg-orange-600 text-white hover:shadow-lg hover:scale-105"
                  }`}
                >
                  <svg
